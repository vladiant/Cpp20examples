...
struct Chat {
  ...
  Chat(Chat&& rhs)
      : mHandle{std::exchange(rhs.mHandle, nullptr)} {}  // #C Move only!
  ...
  void answer(std::string msg)  // #F Send data to the coroutine
                                // and activate it.
  {
...
int main() {
  ...
  std::cout << marco.listen();  // #H Wait for more data from
                                // the coroutine
}
