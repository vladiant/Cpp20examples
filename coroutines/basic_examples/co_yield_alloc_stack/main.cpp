...
struct Task {
  struct promise_type {
    ...
    void* operator new(std::size_t sz, int arg1, int arg2, Buffer& buffer) {
      LOGF();
      log("arguments: ", arg1, " ", arg2, '\n');
      log("size: ", sz, '\n');
      assert(sizeof(buffer.data) >= sz);
      return buffer.data;
    }

    void operator delete(void* ptr, std::size_t sz) { LOGF(); }
  };
...
Task f(int repetitions, int extra, Buffer&) {
  LOGF();
  int a = 0;
  int b = 1;
  co_yield a;
