  ...
  class promise_type {
   public:
    ...
    struct AudioDataAwaiter {
      ...
      // move assignment at client invocation side: const auto data = co_await
      // audioDataResult; This requires that coroutine's result type provides
      // the co_await unary operator
      value_type&& await_resume() const { return std::move(promise_.data_); }
